...
import {GetAllPayeeType,GetAllBanks, LoginAction} from '../Actions/Action';
// DropDownPicker.setListMode("SCROLLVIEW");
const Addpayee = ({navigation, AuthReducer,GetAllPayeeType}) => {
  ...
  const PayeeData =  async () =>{
    Set_Loading(true)
    const Payedata = await GetAllPayeeType();
    // if (Payedata.length > 0 ) {
      Set_Loading(false)
      console.log("Payeedata",Payedata)
    // } else {
    //   console.log("else ka data")
    // }
    // return Payedata
  }
...
export default connect(mapStatestoprop, mapDispatchToprops)(Addpayee);
